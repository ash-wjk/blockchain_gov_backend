const ProjectChain = require('./projectChain');
const WebSocket = require('ws');

const ProjectNode = function(port){
    let projectSockets = [];
    let projectServer;
    let _port = port
    let chain = new ProjectChain();
    ...
    function init(){
        ...
        projectServer = new WebSocket.Server({ port: _port });
		
        projectServer.on('connection', (connection) => {
    ...
    const broadcastMessage = (event, message) => {
        projectSockets.forEach(node => node.send(JSON.stringify({ event, message})))
    }

    const closeConnection = (connection) => {
        console.log('closing connection');
        projectSockets.splice(projectSockets.indexOf(connection),1);
    }
    ...
    const initConnection = (connection) => {
        ...
        projectSockets.push(connection);
